...
//   // CRITICAL FIX: Start with mobile values to match SSR
//   // Always assume mobile/reduced motion for initial render to prevent hydration mismatch
//   const [isMounted, setIsMounted] = useState(false);
//   const [isMobile, setIsMobile] = useState(true); // ✅ Default to true (mobile-first)
//   const [isReducedMotion, setIsReducedMotion] = useState(false);

//   // Only detect actual device characteristics after mount
//   // useEffect(() => {
//   //   setIsMounted(true);
...
//   // Determine the appropriate variant and settings
//   // Use mobile variant until mounted
//   const effectiveVariant = useMemo(() => {
//     if (!isMounted || (isMobile && mergedConfig.mobile.variant)) {
//       return mergedConfig.mobile.variant || mergedConfig.variant;
//     }
//     return mergedConfig.variant;
//   }, [isMounted, isMobile, mergedConfig.variant, mergedConfig.mobile.variant]);

//   const effectiveDuration = useMemo(() => {
//     if (!isMounted || (isMobile && mergedConfig.mobile.duration)) {
//       return mergedConfig.mobile.duration || mergedConfig.duration;
//     }
//     return mergedConfig.duration;
//   }, [isMounted, isMobile, mergedConfig.duration, mergedConfig.mobile.duration]);

//   const effectiveThreshold = useMemo(() => {
//     // Always use mobile threshold until mounted
//     return !isMounted || isMobile ? mergedConfig.threshold.mobile : mergedConfig.threshold.desktop;
//   }, [isMounted, isMobile, mergedConfig.threshold]);
...
const createAnimationVariants = (
  ...
  const variants: Record<AnimationVariant, { parentVariants: Variants; childVariants: Variants }> = {
    ...
    'rotate-in': {
      ...
      childVariants: {
        hidden: { rotate: -12, opacity: 0, scale: 0.9 },
        visible: { rotate: 0, opacity: 1, scale: 1, transition: springTransition },
      },
    },
...
export function useSectionRevealAnimation(config: ScrollAnimationConfig = {}) {
  ...
  const isInView = useInView(ref, { 
    once: mergedConfig.once,
    amount: typeof mergedConfig.amount === 'number' ? mergedConfig.threshold : mergedConfig.amount,
  });
